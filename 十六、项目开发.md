...
这里要注意的是，bs的框架为一个wrapper，一个content，wrapper是滚动条，content是内容，content的高度一定要高于wrapper，于是这里就产生了一个问题，如果内容存在图片，可能会出现DOM渲染时图片未为下载，因此content的高度小于预期，所以我们要在每次数据请求的时候做一次bs.refresh，重新计算滚动距离。
